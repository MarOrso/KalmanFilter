...
def find_F_matrix(x:float,y:float,z:float,q1:float,q2:float,q3:float,q4:float,omega1:float,omega2:float,omega3:float)-> np.array:
    ...
    Om_sqr=(0.000150463)**2
    Om_=0.000150463
    r = np.sqrt( x**2 + y**2 + z**2)
    OM_mat=np.diag([Om_sqr,Om_sqr,0])
    FVV= np.zeros((3,3))
    # First Row
    FVV[0,0] = 0
    FVV[0,1] = 2*Om_
    ...
    tmp2 = np.hstack((J+OM_mat,FVV,Zero_3x3,Zero_3x3))
...
def H_matrix(ix,iy,iz,xcr,ycr,zcr,iq1,iq2,iq3,iq4):
    ...
    norm_rho=sym.sqrt( xc**2 + yc**2 + zc**2)
    ...
    #l=A*(rho/norm_rho)-A*((r_vec-rho)/norm)  #los del cratere
    l=A*((r_vec-rho)/norm)
    lxi=l[0]
    lyi=l[1]
    lzi=l[2]
